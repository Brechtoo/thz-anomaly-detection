...
def rf_split_reconstrucion(df, TARGET_COL):
...
def merge_treshold_logic(rf, X_test, MERGE_HOHLRAUM_CLASSES, USE_THRESHOLDS):
...
if __name__ == "__main__":
    ...
    FEATURE_CSV_PATH = FEATURE_EXTENSION_ROOT
    OUTPUT_PATH = Path("/Users/flo/Desktop/Isenhagen/thz-anomaly-supervised/results/random_forest")

    OUTPUT_DIR = Path(OUTPUT_PATH / "rf_extension_block1")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    OUTPUT_REPORT_PATH = (OUTPUT_DIR / f"rf_extension_block1_{MERGE_HOHLRAUM_CLASSES}_{USE_THRESHOLDS}.txt")

    MARKDOWN_RF_PATH = Path("/Users/flo/Desktop/Isenhagen/thz-anomaly-supervised/docs/overview_rf.md")

    JOBLIB_PATH = Path(OUTPUT_DIR / f"rf_extension_block1_{MERGE_HOHLRAUM_CLASSES}_{USE_THRESHOLDS}.joblib")
    ...
    X, X_train, X_test, y_train, y_test = rf_split_reconstrucion(df, TARGET_COL)
    ...
    y_pred = merge_treshold_logic(rf, X_test, MERGE_HOHLRAUM_CLASSES, USE_THRESHOLDS)
    ...
    with open(MARKDOWN_RF_PATH, "a", encoding="utf-8") as f:
        f.write(
            f"| rf_extension_block1: M {MERGE_HOHLRAUM_CLASSES}, T {USE_THRESHOLDS} "
            f"| {accuracy:.4f} "
            f"| {balanced_acc:.4f} "
            f"| {macro_f1:.4f} "
            f"| {weighted_f1:.4f} "
    ...
    print(f"markdown gespeichert unter: {MARKDOWN_RF_PATH}")
