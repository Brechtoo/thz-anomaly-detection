...
DATA_PATH = Path(FEATURE_BASELINE_ROOT)

OUTPUT_MD_LR = Path("/docs/overview_LR.md")
...
# ============================================================
# Ergebnisse aller Grid-Search-Konfigurationen
# ============================================================
...
grid_results["class_weight"] = (
    grid_results["class_weight"]
    .astype(object)
    .where(grid_results["class_weight"].notna(), "None")
)
...
classes = best_model.named_steps["logistic_regression"].classes_
...
grid_results_text = grid_results.to_string(
    index=False,
    formatters={
        "cv_macro_f1_mean": lambda x: f"{x:.4f}",
        "cv_macro_f1_std": lambda x: f"{x:.4f}",
    },
)
...
OUTPUT_LR_REPORT_PATH= Path("/results/logistic_regression/LR_baseline_report_GRID_CV.txt")
