...
from src.paths import HOHLRAUM_TESTKÖRPER_1_ROOT, LABELS_1_ROOT, RESULTS_ROOT
...
labels = pd.read_csv(
    LABEL_FILE, # dateipfad
    sep=r"\s+", # seperator belibige anzahl von leerzeichen oder tabulatoren
    comment="#", # zeilen die mit # beginnen überspringen (ist hier die erste)
    names=["x", "y", "label"] # kopfzeile wird ignoriert, deshalb selbst spaltennamen vergeben
)

# spaltenreihenfolge explizit setzen
labels = labels[["x", "y", "label"]]

# daten in ganzzahlen umwandeln für datensicherheit
labels["x"] = labels["x"].astype(int)
labels["y"] = labels["y"].astype(int)
labels["label"] = labels["label"].astype(int)

# labeltabelle nach x und y sortieren
labels = labels.sort_values(["x", "y"]).reset_index(drop=True)

# dict für schnellen zugriff, key: (x, y), value: label
label_lookup = {
...
#
# gesucht: [-79.0,-39.0,4.55]
# ergebnis: x = -79 y = -39 z = 4.55
# ============================================================
...
def extract_coordinates(filename: str):
    ...
    # regex liefert string - umwandlung:
...
print(f"CSV gespeichert unter: {OUTPUT_CSV}") # CSV gespeichert unter: /Users/flo/Desktop/Isenhagen/thz-anomaly-supervised/results/data1_labeled.csv
print(f"Anzahl Messungen: {len(df)}") # 21304
print(f"Labels gefunden: {df['label_found'].sum()}") # 21304
print(f"Labels nicht gefunden: {(~df['label_found']).sum()}") # 0
